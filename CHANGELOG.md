...
- **PL_CDR decode gained a strictness `mode` and an opt-in lossless-retention mode**
  ...
  - `deserializeFromPlCdr` now takes `mode: zidl_rt.PlMode`. `.lenient` matches a
    ...
    encoding reads past the parameter's declared length. Contract matches `deserializeInto`:
    `out` must be a fresh `.{}` and on error may be left partially populated for the caller
    to `deinit`. A retaining type returns `error.RetainedOutputNotEmpty` if `out` already
    holds retained parameters (freeing them here could cross allocators).
