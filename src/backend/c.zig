...
const Generator = struct {
    ...
    fn emitStructCdrProtos(self: *Generator, c_name: []const u8, s: *const ir.Struct) !void {
        ...
        // Selective-parse family (see zig.zig's emitSelectiveFns): decode only
        // the members whose bit is set in `_want`, skip the rest.
        if (self.opts.generate_zzdds_wrappers and isZzddsTopicStructC(s)) {
            ...
            try self.print("{s}{s}void {s}_deinit_selected({s} *_v, uint64_t _want);\n", .{ em, sp, c_name, c_name });
        }
        try self.write("\n");
    }
    ...
    fn emitStructZzddsWrapperProtos(self: *Generator, c_name: []const u8) !void {
        ...
        try self.write("    const char *type_name;\n");
        try self.print("}} {s}TypeSupport;\n\n", .{c_name});
        try self.print("typedef struct {s}DataWriter {{\n", .{c_name});
        try self.write("    DDS_DataWriter writer;\n");
        try self.write("    int xcdr_version;\n");
...
/// Returns true if the struct contains any heap-allocating field (recursively
/// through nested structs, typedefs, base structs, and array/sequence element
/// types), indicating it needs a _free function: an unbounded string/wstring
/// (char*/uint16_t*), or a sequence of any bound (sequences are always
/// {_maximum, _length, T*_buffer, _release} in the C mapping — the bound only
...
fn structHasSequenceFields(s: *const ir.Struct) bool {
    if (s.base) |base| {
        switch (base) {
            .struct_ => |bs| if (structHasSequenceFields(bs)) return true,
            else => {},
        }
    }
    for (s.members) |m| {
...
const CdrGenerator = struct {
    ...
    /// Used in _compute_key_hash_from_cdr_key_only to ensure all exit paths
    /// free heap-allocated key fields before returning.
    ...
    fn emitStructFns(self: *CdrGenerator, s: *const ir.Struct) !void {
        ...
        if (has_key) {
            ...
            if (mutable) {
                ...
                // _compute_key_hash_from_cdr (_deserialize_selected-based),
                // not this one.
                for (s.members, 0..) |m, idx| {
            ...
            // _compute_key_hash_from_cdr_key_only: compute key hash from a
            // genuine key-only CDR wire payload (4-byte encapsulation header
            // included) -- RTPS DISPOSE/UNREGISTER, nothing but the @key
            // members. Do not feed this a full sample; see the unified
            // _compute_key_hash_from_cdr (deserialize_selected-based, emitted
            // below alongside the selective-parse family) for that.
            ...
            // narrower key-only case). Decodes the *whole* sample via the
            // already-correct, already-leak-safe _deserialize (so a
            // non-leading @key member is read correctly -- no key-only-decoder
            // skip hazard) and extracts just the key from the result, then
            // frees it. Costs decoding non-key members it doesn't need, but
            // this is a fallback path (only reached when a peer omits the
            // inline key hash), not the hot path -- simplicity and reusing
            // proven decode machinery win over a hand-rolled selective walk
            // here. _v_data starts zeroed, and _free is documented safe on a
            // zeroed-or-partially-decoded value, so freeing it unconditionally
            // (success or failure) needs no goto_label bookkeeping.
            ...
            if (structHasSequenceFields(s)) {
                ...
                try self.print("{s}_free(&_v_data); return _rc; }}\n", .{c_name});
                try self.printI("_rc = {s}_compute_key_hash(&_v_data, _hash);\n", .{c_name});
                try self.printI("{s}_free(&_v_data);\n", .{c_name});
            } else {
                try self.writeI("if (_rc) return _rc;\n");
                try self.printI("_rc = {s}_compute_key_hash(&_v_data, _hash);\n", .{c_name});
            }
        ...
        if (self.opts.generate_zzdds_wrappers and isZzddsTopicStructC(s)) {
            try self.emitSelectiveFnsC(s, c_name, appendable, has_optional);
        }
    ...
    /// Selective-parse family for a Topic struct -- see zig.zig's
    /// `emitSelectiveFns` for the design. `_deserialize_selected` decodes the
    /// members whose bit is set in `_want` and *skips* the rest;
    /// `_deinit_selected` frees exactly the wanted heap members (mirrors
    /// `_free`, mask-gated). `_v` is zeroed first, so a plain `_free` on the
    /// result is also safe. Fallback to `_deserialize` / `_free` for a struct
    /// with a base type or > 64 members (`@mutable` never reaches here).
    fn emitSelectiveFnsC(
        self: *CdrGenerator,
        s: *const ir.Struct,
        c_name: []const u8,
        appendable: bool,
        ...
        try self.printI("*_v = ({s}){{0}};\n", .{c_name});
        if (fallback) {
            try self.writeI("(void)_want;\n");
            try self.printI("return {s}_deserialize(_r, _v);\n", .{c_name});
        } else {
            try self.writeI("int _rc;\n");
            _ = has_optional; // _present is a field of _v, already zeroed above
            if (appendable) {
                try self.writeI("_rc = zidl_cdr_skip_dheader_if_xcdr2(_r);\n");
                try self.writeI("if (_rc) return _rc;\n");
            }
        ...
        // _deinit_selected
        try self.print("void {s}_deinit_selected({s} *_v, uint64_t _want) {{\n", .{ c_name, c_name });
        if (fallback) {
            try self.writeI("(void)_want;\n");
            try self.printI("{s}_free(_v);\n", .{c_name});
        } else {
            var any = false;
            for (s.members, 0..) |m, idx| {
                if (!typeRefHasSequence(m.type_ref)) continue;
                any = true;
                try self.printI("if (_want & (1ull << {d})) {{\n", .{idx});
                self.indent_depth += 1;
                if (m.annotations.is_optional) {
            ...
            if (!any) try self.writeI("(void)_v; (void)_want;\n");
        }
        try self.write("}\n\n");
    }
    ...
    fn emitGetFieldFromCdr(self: *CdrGenerator, s: *const ir.Struct, c_name: []const u8) !void {
        ...
        // filter never needs more than one field per call).
        ...
        if (has_free) {
            try self.printI("if (_rc) {{ {s}_deinit_selected(&_v, _fmask); return false; }}\n", .{c_name});
        } else {
            try self.writeI("if (_rc) return false;\n");
        }
        ...
        if (has_free) try self.printI("{s}_deinit_selected(&_v, _fmask);\n", .{c_name});
    ...
    fn emitStructZzddsWrappers(self: *CdrGenerator, s: *const ir.Struct, c_name: []const u8) !void {
        ...
        try self.writeI("self->writer = writer;\n");
        ...
        // handle; _deserialize_selected decodes just the @key members and
        // skips the rest. Non-key fields of key_out stay zeroed -- unspecified
        // per DDS 1.4 §2.2.2.4.2.17.
        ...
        try self.writeI("DDS_DataReader_return_loan_raw(self->reader, &_c_payloads, &_c_hashes, &_c_infos);\n");
        try self.writeI("return _rc;\n");
        try self.write("}\n\n");
        ...
        try self.print("static int {s}DataReader_decode_batch({s}DataReader *self, DDS_OctetSeqSeq *_payloads, DDS_OctetSeq *_hashes, DDS_SampleInfoSeq *_infos, {s} *values, DDS_SampleInfo *infos) {{\n", .{ c_name, c_name, c_name });
        ...
        if (structHasSequenceFields(s)) {
            ...
            try self.writeI("DDS_DataReader_return_loan_raw(self->reader, _payloads, _hashes, _infos);\n");
            try self.writeI("return _rc;\n");
            self.indent_depth -= 1;
            try self.writeI("}\n");
        } else {
            ...
            try self.writeI("if (_rc) { DDS_DataReader_return_loan_raw(self->reader, _payloads, _hashes, _infos); return _rc; }\n");
        }
        self.indent_depth -= 1;
        try self.writeI("}\n");
        ...
        try self.printI("return {s}DataReader_decode_batch(self, &_c_payloads, &_c_hashes, &_c_infos, values, infos);\n", .{c_name});
        try self.write("}\n\n");
        ...
        // key_hashes is always a plain copy, independent of loan_payloads'
        // loan lifetime (see take_raw's own doc comment) -- released right
        // away here rather than held until the caller's later
        // DataReader_return_loan() call. return_loan_raw is still the only
        // correct way to release it (a generic free can't recover this
        // reader's own allocator); passing zeroed payloads/infos locals
        // limits this call to releasing only _c_hashes, leaving the real
        // loan_payloads/loan_infos untouched.
        try self.writeI("{ DDS_OctetSeqSeq _c_empty_payloads = {0}; DDS_SampleInfoSeq _c_empty_infos = {0}; DDS_DataReader_return_loan_raw(self->reader, &_c_empty_payloads, &_c_hashes, &_c_empty_infos); }\n");
        try self.writeI("if (_n != DDS_RETCODE_OK) return _n;\n");
        ...
        try self.printI("if (!_rc) _rc = info->valid_data ? {s}_deserialize(&_r, out) : {s}_deserialize_key(&_r, out);\n", .{ c_name, c_name });
        try self.writeI("if (_rc) { DDS_OctetSeq _c_no_hashes = {0}; DDS_DataReader_return_loan_raw(self->reader, loan_payloads, &_c_no_hashes, loan_infos); }\n");
        try self.writeI("return _rc;\n");
        try self.write("}\n\n");
        ...
        // See DataReader_take_loaned's matching comment -- key_hashes was
        // already released there, so this always passes an empty one.
        try self.writeI("DDS_OctetSeq _c_no_hashes = {0};\n");
        try self.writeI("DDS_DataReader_return_loan_raw(self->reader, loan_payloads, &_c_no_hashes, loan_infos);\n");
        try self.write("}\n\n");
    }
    ...
    /// Emit `void <CName>_free(<CName> *v)` — frees every heap-owned field
    /// (unbounded string/wstring, any sequence, or a nested struct that itself
    /// needs freeing), recursing into nested structs by delegating to their own
    /// generated `_free`, not by re-walking their members inline. Only called
    /// when `structHasSequenceFields(s)` is true (see that function's doc for
    /// why "sequence" in the name is now a misnomer — it covers strings too).
    fn emitStructFree(self: *CdrGenerator, s: *const ir.Struct) !void {
        const c_name = try self.prefixedCName(s.qualified_name);
        defer self.alloc.free(c_name);
        ...
        if (s.base) |base| {
            switch (base) {
                .struct_ => |bs| if (structHasSequenceFields(bs)) {
                    const base_c = try self.prefixedCName(bs.qualified_name);
                    defer self.alloc.free(base_c);
                    try self.printI("{s}_free(&v->_base);\n", .{base_c});
                },
                else => {},
            }
        }
        for (s.members, 0..) |m, idx| {
            if (!typeRefHasSequence(m.type_ref)) continue;
            const needs_guard = m.annotations.is_optional;
            if (needs_guard) {
                const bit_idx = optBitIdxForMember(s.*, idx);
                try self.printI("if (v->_present & (1ULL << {d}u)) {{\n", .{bit_idx});
                self.indent_depth += 1;
            }
            const lval = try std.fmt.allocPrint(self.alloc, "v->{s}", .{m.name});
            defer self.alloc.free(lval);
            if (m.dimensions.len > 0) {
                try self.emitFreeArrayElementsGeneral(m.type_ref, lval, m.dimensions, 0);
            } else {
    ...
    /// Emit free loops for an array field/element whose element type
    /// (recursively) needs freeing. Recursive to handle multi-dimensional
    /// arrays (e.g. string[2][3], Foo[4] where Foo itself owns heap fields).
    fn emitFreeArrayElementsGeneral(
        self: *CdrGenerator,
        elem_tr: ir.TypeRef,
        lval: []const u8,
        dims: []const u64,
...
test "c_backend: get_field_from_cdr generated for int/float/string members, skips nested" {
    ...
    try testing.expect(has(s, "uint64_t _fmask = Topic_field_index(_field, _field_len, &_fidx) ? (1ull << _fidx) : 0;"));
    try testing.expect(has(s, "_rc = Topic_deserialize_selected(_r, _fmask, &_v);"));
    try testing.expect(has(s, "if (_rc) { Topic_deinit_selected(&_v, _fmask); return false; }"));
    // int-like member
    ...
    try testing.expect(has(s, "Topic_deinit_selected(&_v, _fmask);\n    return _matched;"));
...
test "c_backend cdr: zzdds wrapper implementations for keyed topic" {
    ...
    try testing.expect(has(s, "DDS_ReturnCode_t TopicDataReader_take_loaned(TopicDataReader *self, Topic *out, DDS_SampleInfo *info, DDS_OctetSeqSeq *loan_payloads, DDS_SampleInfoSeq *loan_infos) {"));
    try testing.expect(has(s, "DDS_DataReader_return_loan_raw(self->reader, loan_payloads, &_c_no_hashes, loan_infos);"));
}
...
test "c_backend cdr: _n_impl cleans up partial samples when struct has sequence fields" {
    ...
    try testing.expect(has(s, "DDS_DataReader_return_loan_raw(self->reader, _payloads, _hashes, _infos);"));
...
test "c_backend cdr: _n_impl has no cleanup loop or memset when struct has no sequence fields" {
    ...
    try testing.expect(has(s, "if (_rc) { DDS_DataReader_return_loan_raw(self->reader, _payloads, _hashes, _infos); return _rc; }"));
...
test "c_backend cdr: _decode_batch cleans up partial samples when struct has sequence fields" {
    ...
    const s = out.items;
    try testing.expect(has(s, "for (uint32_t _j = 0; _j <= _i; _j++) Msg_free(&values[_j]);"));
    try testing.expect(has(s, "static int MsgDataReader_decode_batch(MsgDataReader *self, DDS_OctetSeqSeq *_payloads, DDS_OctetSeq *_hashes, DDS_SampleInfoSeq *_infos, Msg *values, DDS_SampleInfo *infos) {"));
    try testing.expect(has(s, "memset(&values[_i], 0, sizeof(values[_i]));"));
}
...
test "c_backend cdr: source file banner and includes" {
    ...
    // Generated code only ever calls zidl_cdr_alloc/_free/_strdup (declared in
    // zidl_cdr.h, already included above) — never a bare libc allocation
    // function — so <stdlib.h> is not needed and would be actively
    // unwelcome on the freestanding/embedded toolchains this allocator work
    // targets.
...
test "c_backend cdr: compute_key_hash_from_cdr_key_only frees @key unbounded string" {
    // Greptile PR #5: _compute_key_hash_from_cdr_key_only must free heap allocations
...
test "c_backend cdr: compute_key_hash_from_cdr_key_only with @key sequence<long> frees buffer only" {
...
test "c_backend cdr: compute_key_hash_from_cdr_key_only with @key sequence<string> frees elements then buffer" {
...
test "c_backend cdr: compute_key_hash_from_cdr_key_only with @key string array frees each element" {
...
test "c_backend cdr: compute_key_hash_from_cdr_key_only with only primitive key has no cleanup" {
...
test "c_backend cdr: compute_key_hash_from_cdr (unsuffixed) uses full deserialize, not the key-only decoder" {
    ...
    var c_src = try testGenCdr("@appendable struct Msg { string label; @key long id; };", "msg");
    defer c_src.deinit(testing.allocator);
    const s = c_src.items;
    ...
    try testing.expect(has(s, "Msg_compute_key_hash(&_v_data, _hash)"));
    try testing.expect(has(s, "Msg_free(&_v_data)"));
    // Must not fall back to the key-only decoder for the full-payload path.
    const from_cdr = std.mem.indexOf(u8, s, "int Msg_compute_key_hash_from_cdr(").?;
    const from_cdr_end = std.mem.indexOfPos(u8, s, from_cdr, "\n}\n").?;
    try testing.expect(!has(s[from_cdr..from_cdr_end], "Msg_deserialize_key"));
}

test "c_backend cdr: compute_key_hash_from_cdr frees the full decode on failure, not just key fields" {
    var c_src = try testGenCdr("@appendable struct Msg { @key string tag; sequence<long> nums; };", "msg");
    defer c_src.deinit(testing.allocator);
    const s = c_src.items;
    const from_cdr = std.mem.indexOf(u8, s, "int Msg_compute_key_hash_from_cdr(").?;
    const from_cdr_end = std.mem.indexOfPos(u8, s, from_cdr, "\n}\n").?;
    const body = s[from_cdr..from_cdr_end];
    // Msg_free (the whole-struct free) must run on both the error path and
    // the success path -- Msg_deserialize does not self-clean, and _v_data
    // starts zeroed so a partial decode is always safe to free.
    try testing.expect(has(body, "if (_rc) { Msg_free(&_v_data); return _rc; }"));
    try testing.expect(has(body, "_rc = Msg_compute_key_hash(&_v_data, _hash);"));
    try testing.expect(has(body, "Msg_free(&_v_data);"));
}

test "c_backend cdr: compute_key_hash_from_cdr skips the free call when nothing needs freeing" {
    var c_src = try testGenCdr("@final struct Msg { @key long id; double value; };", "msg");
    defer c_src.deinit(testing.allocator);
    const s = c_src.items;
    const from_cdr = std.mem.indexOf(u8, s, "int Msg_compute_key_hash_from_cdr(").?;
    const from_cdr_end = std.mem.indexOfPos(u8, s, from_cdr, "\n}\n").?;
    try testing.expect(!has(s[from_cdr..from_cdr_end], "Msg_free"));
}

test "c_backend cdr: @final struct with non-leading key has no _Static_assert (guard removed)" {
    ...
    // non-key bytes), and a full-payload caller has its own correctly-skipping
    // function (compute_key_hash_from_cdr) to reach for instead.
    var c_src = try testGenCdr("@final struct Msg { string label; @key long id; };", "msg");
    defer c_src.deinit(testing.allocator);
    const s = c_src.items;
    try testing.expect(!has(s, "_Static_assert"));
...
test "c_backend cdr: @default string with non-ASCII uses octal escape" {
    ...
    var src = try testGenCdr(idl, "cfg");
    defer src.deinit(testing.allocator);
    try testing.expect(has(src.items, "zidl_cdr_strdup(\"\\001\")"));
}
...
test "c_backend: struct with only primitives/bounded string has no _free declaration" {
    const idl =
        \\struct Simple { long x; string<16> name; };
    ;
    var h = try testGen(idl, "t");
    defer h.deinit(testing.allocator);
...
test "c_backend: struct with only an unbounded string field gets _free declared and defined" {
    const idl =
    ...
    try testing.expect(has(src.items, "zidl_cdr_free_str(v->name);"));
}

test "c_backend: struct with only a bounded sequence field gets _free declared and defined" {
    const idl =
        \\struct Simple { long x; sequence<long, 4> items; };
    ;
    var h = try testGen(idl, "t");
    defer h.deinit(testing.allocator);
    try testing.expect(has(h.items, "void Simple_free(Simple *v);"));
...
test "c_backend: --c-no-free + --generate-zzdds-wrappers still calls Msg_free from get_field_from_cdr (no leak on the CFT evaluation path)" {
    ...
    try testing.expect(!has(s.items, "void Msg_free(Msg *v) {"));
    // ...and get_field_from_cdr still releases what it deserialized on both
    // the early-return (deserialize failure) and normal-completion paths --
    // now via the always-generated Msg_deinit_selected (whose body frees the
    // sequence inline, so it no longer depends on Msg_free's existence at
    // all -- the original --c-no-free leak can't recur).
    try testing.expect(has(s.items, "Msg_deinit_selected(&_v, _fmask)"));
}
