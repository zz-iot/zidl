...
const ConcreteImplGenerator = struct {
    ...
    fn emit(self: *ConcreteImplGenerator, spec: *const ir.Spec) !void {
        ...
        // <mutex>/<unordered_map> back _getOrCreate's identity cache, emitted only when
        // the pre-scan above found at least one interface that actually needs it —
        // skip the includes entirely otherwise (e.g. struct-only IDL, or a spec whose
        // entity interfaces are never wrapped anywhere), rather than always paying for
        // STL headers no emitted code in this file ends up using.
