...
test "cpp_backend: entity wrapper identity — _getOrCreate declared and cache mutex-guarded" {
    ...
    const hdr = res.hdr.items;
    const src = res.src.items;
    // Public factory declared alongside the constructor, so callers never
    // construct BarImpl directly and bypass the cache.
    ...
    try testing.expect(has(src, "_cache[h] = _sp;"));
}
