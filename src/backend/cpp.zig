...
const ConcreteImplGenerator = struct {
    ...
    fn emit(self: *ConcreteImplGenerator, spec: *const ir.Spec) !void {
        ...
        // <mutex>/<unordered_map> back _getOrCreate's identity cache, emitted only for
        // entity (non-callback) interfaces — skip the includes entirely for spec files
        // with none (e.g. struct-only IDL), rather than always paying for STL headers
        // no emitted code in this file ends up using.
        {
            ...
            try self.collectModuleInterfaces(spec.items, &entities, &callbacks);
...

        {
            ...
            try self.srcPrint(
                "std::shared_ptr<{s}Impl> {s}Impl::_getOrCreate({s} h) {{\n" ++
                    ...
                    "    }}\n" ++
                    "    auto _sp = std::make_shared<{s}Impl>(h);\n" ++
                    "    if (_it != _cache.end()) {{\n" ++
                    "        _it->second = _sp;\n" ++
                    "    }} else {{\n" ++
                    ...
                    "    return _sp;\n" ++
                    "}}\n\n",
                .{ iface.name, iface.name, c_name, c_name, iface.name, iface.name },
            );
        }
                    ...
                    .named => |td| switch (td) {
                        .interface => |piface| {
                            if (!isCallbackIface(piface)) {
                                ...
                                try self.srcPrint("{s}Impl::_getOrCreate({s})", .{ piface.name, p.name });
                            } else {
...
test "cpp_backend: B1+B3 — entity return wraps in Impl" {
    ...
    try testing.expect(has(src, "make_shared<BarImpl>(h)"));
}
...
test "cpp_backend: _getOrCreate has no entity interfaces — mutex/unordered_map includes are skipped" {
    var res = try testGenConcreteImpl(
        \\module DDS {
